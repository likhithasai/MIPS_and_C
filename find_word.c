...
void read_string(char* s, int size) { fgets(s, size, stdin); }    // Do we really need this?

void print_char(char c)    { printf("%c", c); }         //Done
void print_int(int num)    { printf("%d", num); }       //Done
void print_string(const char* s) { printf("%s", s); }   //Done
...
#define MAX_SENTENCE_LENGTH 1000  // Done

// Maximum characters in a word excluding terminating null character
#define MAX_WORD_LENGTH 50      //Done

// Global variables
// +1 to store terminating null character
char input_sentence[MAX_SENTENCE_LENGTH+1];     // Done
char word[MAX_WORD_LENGTH+1];                   // Done
// Read_input function is done for now******************************************
void read_input(const char* inp) {              // Done
    print_string("\nEnter input: ");
    read_string(input_sentence, MAX_SENTENCE_LENGTH+1);
}

void output(const char* out) {                //Done
    print_string(out);                        // Done
    print_string("\n");                       // Done
}
...
// Retains its value across function cError in /afs/inf.ed.ac.uk/user/s16/s1684168/Documents/CS/MIPS/read_input.s line 41: Runtime exception at 0x00400040: address out of range 0x000003e9

int input_index = 0;

// Marks if the sentence is processed
int end_of_sentence = false;   //Done
...
int main() {
    ...
    int word_found = false;   // Done

    read_input(input_sentence); //Done

    print_string("\noutput:\n");  //Done

    do {

        word_found = process_input(input_sentence, word);    // Done

        if ( word_found == true ) {
            output(word);               // Done
        }

    } while ( word_found == true );     //Done
